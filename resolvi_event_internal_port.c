...
static uint32_t resolvi_event_device_setup_internal_port(
  ...
  if (ret < 0) rte_panic("Error in configuring event device\n");

  evt_rsrc->event_d_id = event_d_id;
  return event_queue_cfg;
}

static void resolvi_event_port_setup_internal_port(
    struct resolvi_resources *rsrc) {
  struct resolvi_event_resources *evt_rsrc = rsrc->evt_rsrc;
  ...
  if (ret < 0) rte_panic("Error getting default configuration of Event Port\n");
  ...
  for (event_p_id = 0; event_p_id < evt_rsrc->evp.nb_ports; event_p_id++) {
    ...
    //?: Nothing is linked?
    ret = rte_event_port_link(event_d_id, event_p_id, NULL, NULL, 0);
    if (ret < 0)
...
static void resolvi_event_queue_setup_internal_port(
    struct resolvi_resources *rsrc, uint32_t event_queue_cfg) {
  struct resolvi_event_resources *evt_rsrc = rsrc->evt_rsrc;
  uint8_t event_d_id = evt_rsrc->event_d_id;
  ...
  ret = rte_event_queue_default_conf_get(event_d_id, event_q_id, &def_q_conf);
  if (ret < 0) rte_panic("Error getting default config of event queue\n");
  ...
  if (!evt_rsrc->evq.event_q_id)
    rte_panic("Failed to allocate memory for event queue id\n");

  /* Configure atomic queues 8< */
  for (event_q_id = 0; event_q_id < evt_rsrc->evq.nb_queues; event_q_id++) {
    ret = rte_event_queue_setup(event_d_id, event_q_id, &event_q_conf);
    if (ret < 0) rte_panic("Error in setting up event queue %d\n", event_q_id);
    evt_rsrc->evq.event_q_id[event_q_id] = event_q_id;
  }
  /* >8 End of configuring atomic queues. */
}

static void resolvi_rx_tx_adapter_setup_internal_port(
    struct resolvi_resources *rsrc) {
  struct resolvi_event_resources *evt_rsrc = rsrc->evt_rsrc;
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    ret = rte_event_eth_rx_adapter_create(adapter_id, event_d_id,
                                          &evt_rsrc->def_p_conf);
    if (ret) rte_panic("Failed to create rx adapter[%d]\n", adapter_id);
    ...
    ret = rte_event_eth_rx_adapter_queue_add(adapter_id, port_id, -1,
                                             &eth_q_conf);
    if (ret) rte_panic("Failed to add queues to Rx adapter\n");

    ret = rte_event_eth_rx_adapter_start(adapter_id);
    if (ret) rte_panic("Rx adapter[%d] start Failed\n", adapter_id);

    evt_rsrc->rx_adptr.rx_adptr[adapter_id] = adapter_id;
    adapter_id++;
    if (q_id < evt_rsrc->evq.nb_queues) q_id++;
  }
  ...
  RTE_ETH_FOREACH_DEV(port_id) {
    ...
    ret = rte_event_eth_tx_adapter_create(adapter_id, event_d_id,
                                          &evt_rsrc->def_p_conf);
    if (ret) rte_panic("Failed to create tx adapter[%d]\n", adapter_id);

    ret = rte_event_eth_tx_adapter_queue_add(adapter_id, port_id, -1);
    if (ret) rte_panic("Failed to add queues to Tx adapter\n");

    ret = rte_event_eth_tx_adapter_start(adapter_id);
    if (ret) rte_panic("Tx adapter[%d] start Failed\n", adapter_id);
...
void resolvi_event_set_internal_port_ops(struct event_setup_ops *ops) {
  ops->event_device_setup = resolvi_event_device_setup_internal_port;
  ops->event_queue_setup = resolvi_event_queue_setup_internal_port;
  ops->event_port_setup = resolvi_event_port_setup_internal_port;
  ops->adapter_setup = resolvi_rx_tx_adapter_setup_internal_port;
}